...
def pushplus_post(title, content, pushplus_token):
...
def unchecked_list_health(date, jwsession, pushplus_token):
    headers = {
        "jwsession": jwsession,
    ...
    for t in ts['data']:
        ...
        print(t['name'])
    pushplus_post("班级健康打卡提醒", "未打卡名单:" + name_list, pushplus_token)


def unchecked_list_morning(date, jwsession, pushplus_token):
    headers = {
        "jwsession": jwsession,
        'user-agent': user_agent
    }
    ...
    if ts['code'] != -10:
        ...
        for t in ts['data']:
            ...
            morning_check_for_classmate(t['userId'],jwsession)
            time.sleep(2)
        pushplus_post("班级晨检未打卡提醒", "未打卡名单:" + name_list, pushplus_token)
    else:
        pushplus_post("班级晨检未打卡提醒", "jwsession可能失效!", pushplus_token)


def unchecked_list_afternoon(date, jwsession, pushplus_token):
    headers = {
        "jwsession": jwsession,
        'user-agent': user_agent
    }
    ...
    if ts['code'] != -10:
        ...
        for t in ts['data']:
            ...
            afternoon_check_for_classmate(t['userId'],jwsession)
            time.sleep(2)
        pushplus_post("班级午检未打卡提醒", "未打卡名单:" + name_list, pushplus_token)
    else:
        pushplus_post("班级午检未打卡提醒", "jwsession可能失效!", pushplus_token)


def health_check_in(jwsession, pushplus_token):
    headers = {
        "jwsession": jwsession,
        'user-agent': user_agent
    }
    ...
    if r_json['code'] == 0:
        pushplus_post("个人健康打卡打卡提醒", "打卡成功", pushplus_token)
    else:
        pushplus_post("个人健康打卡打卡提醒", "打卡失败,返回信息为:{0}".format(str(r_json)), pushplus_token)


def morning_check(jwsession, pushplus_token):
    headers = {
        "jwsession": jwsession,
        'user-agent': user_agent
    }
    ...
    r_json = json.loads(r.text)
    if r_json['code'] == 0:
        pushplus_post("个人晨检提醒", "打卡成功", pushplus_token)
    else:
        pushplus_post("个人晨检提醒", "打卡失败,返回信息为:" + str(r_json), pushplus_token)


def noon_inspection(jwsession, pushplus_token):
    headers = {
        "jwsession": jwsession,
        'user-agent': user_agent
    }
    ...
    r_json = json.loads(r.text)
    if r_json['code'] == 0:
        pushplus_post("个人午检打卡提醒", "打卡成功", pushplus_token)
    else:
        pushplus_post("个人午检打卡提醒", "打卡失败,返回信息为:" + str(r_json), pushplus_token)


def morning_check_for_classmate(userid, jwsession):
    headers = {
        "jwsession": jwsession,
        'user-agent': user_agent
    }
...
def afternoon_check_for_classmate(userid, jwsession):
    headers = {
        "jwsession": jwsession,
        'user-agent': user_agent
    }
...
##############################################################
# 晚上定位签到
def get_sign_message(jwsession, pushplus_token):
    headers = {
        "jwsession": jwsession
    }
    post_data = {
    ...
    if r_json['code'] == 0:
        ...
        pushplus_post("签到提醒", "jwsession失效!", pushplus_token)
        return 404


def do_sign(sign_message, jwsession, pushplus_token):
    if sign_message == 404:
        return 404
    headers = {
        "jwsession": jwsession
    ...
    r_json = json.loads(r.text)
    if r_json['code'] == 0:
        pushplus_post("签到提醒", "签到成功", pushplus_token)
    else:
        pushplus_post("签到提醒", "签到失败,返回信息为:" + str(r_json), pushplus_token)
...
def leader_sign(jwsession, not_sign_list, flag=1):
...
# 打卡事件操作
def operate_event(jwsession, pushplus_token, flag=1):
    # 设立标志位用于区分周末，如果周末就不打卡，只是返回名单
    while True:
        res, sign_id = contrast_date_class(get_list(jwsession))
        if res == 0:
            not_sign_list = get_sign_result(sign_id, jwsession)
            if not_sign_list != 404:
                not_sign_name = leader_sign(jwsession, not_sign_list, flag)
                if len(not_sign_name) != 0:
                    name_str = ""
                    for name in not_sign_name:
                        name_str = name_str + name + " "
                    pushplus_post("今日班级打卡情况", "未打卡名单:" + name_str, pushplus_token)
                else:
                    pushplus_post("今日班级打卡情况", "已经全部打卡", pushplus_token)
                time.sleep(10)
                break
        # 签到是今天但是签到没有开始，静默等待
        elif res == -2:
            ...
            pushplus_post("今日班级打卡情况", "已过签到时间", pushplus_token)
            break
        elif res == -1:
            pushplus_post("今日班级打卡情况", "签到未发布或今天没有签到", pushplus_token)
            break
...
def main():
    while True:
        ...
        if time_now == "06:30:10" or time_now == "06:30:11":
            infos = readfile()
            for info in infos:
                print("正在给{}的{}晨检".format(info["class"], info["name"]))
                morning_check(info["jwsession"], info["pushplus_token"])
        # 午检
        if time_now == "11:05:00" or time_now == "11:05:01":
            infos = readfile()
            for info in infos:
                print("正在给{}的{}午检".format(info["class"], info["name"]))
                noon_inspection(info["jwsession"], info["pushplus_token"])

        # 晨检未打卡名单并提醒
        if time_now == "09:40:00" or time_now == "09:40:01":
            time_data = time.strftime("%Y%m%d")
            infos = readfile()
            for info in infos:
                print("正在给{}的{}班级晨检".format(info["class"], info["name"]))
                unchecked_list_morning(time_data, info["jwsession"], info["pushplus_token"])  # 检查两次，会出现第一次打卡不完整的情况

        # 午检未打卡名单并提醒
        if time_now == "14:40:00" or time_now == "14:40:01":
            time_data = time.strftime("%Y%m%d")
            infos = readfile()
            for info in infos:
                print("正在给{}的{}班级午检".format(info["class"], info["name"]))
                unchecked_list_afternoon(time_data, info["jwsession"], info["pushplus_token"])
        ...
        if time_now == "21:40:00" or time_now == "21:40:01":
            ...
            for info in infos:
                print("正在给{}的{}班级午检".format(info["class"], info["name"]))
                sign_info = get_sign_message(info["jwsession"], info["pushplus_token"])
                if sign_info == 404:
                    time.sleep(4)
                    continue
                # 比对签到信息
                ...
                if time_code == 0:
                    do_sign(sign_info, info["jwsession"], info["pushplus_token"])
                    time.sleep(10)
                elif time_code == -2:
                    # 签到是今天但是签到没有开始，静默等待
                    while time_code == 0:
                    ...
                    pushplus_post("签到提醒", "已过签到时间", info["pushplus_token"])
                elif time_code == -1:
                    pushplus_post("签到提醒", "签到未发布或今天没有签到", info["pushplus_token"])
        ...
        if week_day_now != "0":
            ...
            if time_now == "22:40:00" or time_now == "22:40:01":
                ...
                for info in infos:
                    print("正在给{}的{}班级午检".format(info["class"], info["name"]))
                    operate_event(info["jwsession"], info["pushplus_token"])  # 打卡
        # 星期天的情况下
        else:
            time_now = time.strftime("%H:%M:%S", time.localtime())
            if time_now == "22:40:00" or time_now == "22:40:01":
                ...
                for info in infos:
                    print("正在给{}的{}班级午检".format(info["class"], info["name"]))
                    operate_event(info["jwsession"], info["pushplus_token"], 2)  # 返回名单不打卡
        time.sleep(2)  # 停两秒
